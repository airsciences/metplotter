...
try{o.open("PUT",t,this.async),o.setRequestHeader("Authorization",this.getAccessToken()),o.setRequestHeader("Content-Type","application/json;charset=UTF-8"),o.send(i)}catch(t){n=t,console.log(s+"catch(error).",n)}},t.prototype.post=function(t,i,e){var n,s,o
...
for(e in n)o=n[e],t=!1,this.state.range.data[e].max.getTime()<i.getTime()-9e6&&(t=this.state.interval.data[e].max<this.options.requestInterval.data),this.state.request.data[e]={min:this.state.interval.data[e].min<this.options.requestInterval.data,max:t},null==this.state.requested.data[e]?s.push(this.state.requested.data[e]={min:!1,max:!1}):s.push(void 0)
...
return i=t.setZoomTransform(),t.plotter.i.zoom.set(i)})},t.prototype.setBandDomain=function(t){return this.definition.x1=t},t.prototype.calculateChartDims=function(){var t,i,e
return e=null!=this.options.width?Math.round(this.options.width):Math.round($(this.options.target).width())-24,t=Math.round(e/this.options.aspectDivisor),e>1e3?i={top:Math.round(.04*t),right:Math.round(Math.pow(e,.3)),bottom:Math.round(.12*t),left:Math.round(Math.pow(e,.6))}:e>600?(this.device="mid",this.options.font.size=this.options.font.size/1.25,t=Math.round(e/(this.options.aspectDivisor/1.25)),i={top:Math.round(.04*t),right:Math.round(Math.pow(e,.3)),bottom:Math.round(.14*t),left:Math.round(Math.pow(e,.6))}):(this.device="small",this.options.font.size=this.options.font.size/1.5,t=Math.round(e/(this.options.aspectDivisor/1.5)),i={top:Math.round(.04*t),right:Math.round(Math.pow(e,.3)),bottom:Math.round(.18*t),left:Math.round(Math.pow(e,.6))}),this.definition.dimensions={width:e,height:t,margin:i},this.definition.dimensions.topPadding=parseInt(this.definition.dimensions.margin.top),this.definition.dimensions.bottomPadding=parseInt(this.definition.dimensions.height-this.definition.dimensions.margin.bottom),this.definition.dimensions.leftPadding=parseInt(this.definition.dimensions.margin.left),this.definition.dimensions.innerHeight=parseInt(this.definition.dimensions.height-this.definition.dimensions.margin.bottom-this.definition.dimensions.margin.top),this.definition.dimensions.innerWidth=parseInt(this.definition.dimensions.width-this.definition.dimensions.margin.left-this.definition.dimensions.margin.right),this.definition.x=d3.scaleTime().range([i.left,e-i.right]),this.definition.x1=d3.scaleBand().rangeRound([i.left,e-i.right],.05).padding(.1),this.definition.y=d3.scaleLinear().range([t-i.bottom,i.top])},t.prototype.calculateAxisDims=function(t){return this.calculateXAxisDims(t),this.calculateYAxisDims(t)},t.prototype.calculateXAxisDims=function(t){return null===this.options.x.min?this.definition.x.min=d3.min(t[0],function(t){return t.x}):this.definition.x.min=this.parseDate(this.options.x.min),null===this.options.x.max?this.definition.x.max=d3.max(t[0],function(t){return t.x}):this.definition.x.max=this.parseDate(this.options.x.max)},t.prototype.calculateYAxisDims=function(t){var i,e,n,s
...
for(s in r)l=r[s],this.bars[s]=this.barWrapper.append("g").attr("clip-path","url(#"+this.options.target+"_clip)").selectAll(".bar-"+s).data(l).enter().append("rect").attr("class","bar-"+s).attr("x",function(i){return t.definition.x(i.x)}).attr("width",d3.max([1,this.definition.x1.bandwidth()])).attr("y",function(i){return t.definition.y(i.y)}).attr("height",function(i){return t.definition.dimensions.innerHeight+t.definition.dimensions.margin.top-t.definition.y(i.y)}).style("fill",this.options.y[s].color)
...
return this.appendCrosshairTarget(n),n}},t.prototype.setCrosshair=function(t,i){var e,n,s,o,r,a,l,p,d,h,u,c,f,m,g
if(this.initialized){c=this.preError+".setCrosshair(mouse)",e=this,s=this.definition.dimensions,l=function(t){switch(!1){case!(t>360||t<0):return"INV"
...
for(u in f)m=f[u],o=this.overlay.node(),n=m,i=i?i:d3.mouse(o),g=this.definition.x.invert(i[0]+s.leftPadding),t&&(g=this.definition.x.invert(t.invertX(i[0]+s.leftPadding))),h=e.bisectDate(n,g,1),g.getTime()<this.state.range.data[u].min.getTime()&&h--,g.getTime()>this.state.range.data[u].max.getTime()&&h--,h=g.getMinutes()>=30?h:h-1,null!=n[h]&&(p=t?t.applyX(this.definition.x(n[h].x)):this.definition.x(n[h].x),d=[],r=[],null!==this.options.y[u].variable&&(r[u]=n[h],null!=r[u]&&(d[u]=this.definition.y(r[u].y),isNaN(d[u])||null==r[u].y||this.focusRect[u].attr("transform","translate(0, 0)"))),a=p-s.leftPadding,a>=0&&(this.crosshairs.select(".crosshair-x").attr("x1",a).attr("y1",s.topPadding).attr("x2",a).attr("y2",s.innerHeight+s.topPadding).attr("transform","translate("+s.leftPadding+", 0)"),this.crosshairs.select(".crosshair-x-under").attr("x",a).attr("y",s.topPadding).attr("width",s.innerWidth-a).attr("height",s.innerHeight).attr("transform","translate("+s.leftPadding+", 0)")),null===this.options.y[u].variable||isNaN(d[u])||null==r[u].y||(this.focusRect[u].attr("width",t.k*this.definition.x1.bandwidth()).attr("x",p).attr("y",d[u]),this.focusText[u].attr("x",p+s.leftPadding/10+t.k*this.definition.x1.bandwidth()+2).attr("y",d[u]-s.topPadding/10).text(null!=r[u].y?"wind_direction"===e.options.y[0].variable?l(r[u].y):r[u].y.toFixed(2)+" "+this.options.y[u].units:void 0)))
...
return a=location.protocol+"//dev.nwac.us/api/v5/dataloggerregion?sensor_name="+e,n=this,s={},r=this.getCurrent(t),l=this.plotter.lib.uuid(),o=function(e){var s,o,r,a,p,d,h,u,c,f
for(n.stations[t]=e.responseJSON.results,o='<div class="dropdown"> <li><a id="'+l+'" class="station-dropdown dropdown-toggle" role="button" title="Select Stations" data-toggle="dropdown" href="#"> <i class="icon-list"></i></a> <ul id="station-dropdown-'+t+'" class="dropdown-menu pull-right">',h=n.stations[t],r=0,p=h.length;r<p;r++){for(c=h[r],s=n.plotter.lib.toLower(c.name),o=o+' <li class="subheader"> <a data-region="'+s+'" data-plot-id="'+t+'" href=""> <i class="icon-caret-down" style="margin-right: 6px"></i> <span class="region-name">'+c.name+'</span> <span class="region-dots"></span> </a> </li> <ul class="list-group-item sublist" style="display: none;">',u=c.dataloggers,a=0,d=u.length;a<d;a++)f=u[a],o=o+' <li class="station" data-station-id="'+f.id+'" data-plot-id="'+t+'" style="padding: 1px 5px; cursor: pointer; list-style-type: none"> <i class="icon-circle"></i> <span class="station-name"> '+f.datalogger_name+" | "+f.elevation+" ft </span> </li>"
o+=" </ul>"}return o+=" </ul> </li>",$(i).prepend(o),$("#"+l).dropdown(),n.bindSubMenuEvent(".subheader"),n.setCurrent(t),n.updateStationDropdown(t),n.appendStationMap(t,i,e.responseJSON.results)},this.api.get(a,s,o)},t.prototype.updateStationDropdown=function(t){var i,e,n,s,o,r,a,l,p,d,h,u,c,f,m
...
return t.stopPropagation(),e=$(this).attr("data-plot-id"),$(this).append('<i class="icon-spinner icon-spin" data-plot-id="'+e+'"></i>'),i.addStation(e,$(this).attr("data-station-id"))})},u=this.stations[t],l=0,d=u.length;l<d;l++){for(f=u[l],o=this.plotter.lib.toLower(f.name),r="",a="",s="",f.displayed.length>0&&(s="rgb(248, 248, 248)",a=700,r=n(f.displayed)),c=f.dataloggers,p=0,h=c.length;p<h;p++)m=c[p],$('[data-station-id="'+m.id+'"][data-plot-id="'+t+'"]> i.icon-circle').css("color",m.color),e(t,i.plotter,m)
...
return o},t.prototype.updateStationMap=function(t){var i,e,n,s,o,r,a,l,p
...
for(o in r)l=r[o],null!==l.variable?(e=l.variable.replace("_","-"),s="map-plot-"+t+"-station-"+l.dataLoggerId,a.push(p(t,s,l.color))):a.push(void 0)
return a},t.prototype.boundOnSelected=function(t){var i,e,n,s,o,r,a,l,p,d
i=this,n=new google.maps.LatLngBounds,e=[],d=this.markers[t]
for(s in d)o=d[s],a=o.get("selected"),a===!0&&e.push(o.getPosition())
...
return o=this.plotter.plots[t].proto.options.uuid,e=128,n=256,"http://localhost:5000"===location.origin&&(e=0,n=0),i=this.plotter.i.controls.maps[t].getCenter(),r=this.plotter.i.controls.maps[t].getZoom(),s=$("#map-control-"+o).parent().parent().prev().offset(),$("#map-control-"+o).parent().parent().toggle().css("left",s.left-356-e).css("top",s.top-n),google.maps.event.trigger(this.plotter.i.controls.maps[t],"resize"),this.plotter.i.controls.maps[t].setCenter(i),this.plotter.i.controls.maps[t].setZoom(r)},t.prototype.toggle=function(t){return $(t).toggle()},t.prototype.move=function(t,i,e){var n,s,o
return n=this,s="up"===e?"Up":"Down",o='<i id="move-'+t+"-"+e+'" style="cursor: pointer;" title="Move Plot '+s+'" class="icon-arrow-'+e+'"></i>',$(i).append(o),$("#move-"+t+"-"+e).on("click",function(){return n.plotter.move(t,e)})},t.prototype.remove=function(t,i){var e,n
return e=this,n='<i id="remove-'+t+'" style="cursor: pointer;" title="Remove Plot" class="icon-remove"></i>',$(i).append(n),$("#remove-"+t).on("click",function(){return e.plotter.remove(t)})},t.prototype.new=function(t){var i,e,n,s,o,r,a,l,p,d,h
for(i=this,h=this.plotter.lib.uuid(),e=[{variable:"battery_voltage",title:"Battery Voltage"},{variable:"net_solar",title:"Solar Radiation"},{variable:"solar_pyranometer",title:"Solar Pyranometer"},{variable:"relative_humidity",title:"Relative Humidity"},{variable:"barometric_pressure",title:"Barometric Pressure"},{variable:"snow_depth",title:"Snow Depth"},{variable:"snowfall_24_hour",title:"24-Hour Snowfall"},{variable:"intermittent_snow",title:"Intermittent Snow"},{variable:"wind_direction",title:"Wind Direction"},{variable:"precipitation",title:"Precipitation (Bar Plot)"},{variable:"precipitation_line",title:"Precipitation (Line Plot)"},{variable:"temperature",title:"Temperature"},{variable:"equip_temperature",title:"Equipment Temperature"},{variable:"wind_speed_average",title:"Wind Speed"}],n='<ul id="new-'+h+'-dropdown" class="dropdown-menu pull-right" role="menu" aria-labelledby="new-'+h+'"> <li><a id="new-'+h+'-parameter" style="cursor: pointer">Add Parameter Plot</a></li> <li><a id="new-'+h+'-station" style="cursor: pointer">Add Station Plot</a></li> </ul>',s='<div class="dropdown"> <li><a id="new-'+h+'" class="dropdown-toggle" title="Add New Plot" data-toggle="dropdown" role="button" href="#"> <i class="icon-plus"></i></a> <ul class="dropdown-menu pull-right" role="menu" aria-labelledby="'+h+'">',o=0,a=e.length;o<a;o++)d=e[o],s=s+' <li><a id="new_'+d.variable+"_"+h+'" data-variable="'+d.variable+'">'+d.title+"</a></li>"
for(s+=" </ul> </li> </div>",$(t).append(s),p=[],r=0,l=e.length;r<l;r++)d=e[r],p.push($("#new_"+d.variable+"_"+h).off("click").on("click",function(){var t
...
t[p]instanceof Date?t[p].getTime()===e[p].getTime()&&s++:t[p]===e[p]&&s++}return s===o&&(l=!0),l})):void console.log(i+" data not of type array.")}return t.prototype.join=function(t,e){var n,s,o,r,a,l,p,d,h,u,c,f,m,g,y
...
for(s in d)if(p=d[s],u=this.test(y,p,e)){for(h in p)c=p[h],i.call(e,h)<0&&(g[o-1][h+r]=c)
d.splice(s,1)
break}}return this.sourceCount++,this.data=this._clean(g),this.data},t.prototype.merge=function(t,e){var n,s,o,r,a,l,p,d,h,u,c,f,m,g
...
for(s in p)if(l=p[s],h=this.test(g,l,e)){for(d in l)u=l[d],i.call(e,d)<0&&(m[o-1][d]=u)
p.splice(s,1)
break}}return this.data=this._clean(m),this.data},t.prototype.append=function(t,e){var n,s,o,r,a,l,p,d,h,u,c
h=this.preError+".append(data, onKeys)",s=$.extend(!0,[],this.data),r=$.extend(!0,[],t)
for(d in s){c=s[d]
for(n in r)if(o=r[n],l=this.test(c,o,e)){for(a in o)p=o[a],i.call(e,a)<0&&(s[d][a]=p)
...
return a},t.prototype.add=function(t){var i,e,n,s,o,r
return i=this.plotter.i.template.template[t],e=this.plotter.plots[0].proto.getState(),o=this.plotter.lib.format(e.range.data[0].max),s=e.length.data[0],n=this.plotter.i.template.forSync(t,0,o,s),r=this.plotter.lib.uuid(),this.requests[r]={plotId:t,ready:!1,requested:!1},this.requests[r].requested=this.getAppend(t,0,r,n)},t.prototype.get=function(t,i,e,n){var s,o,r,a
...
return r=this.preError+".get()",a=this.endpoint(),s=this,o=function(n){if(null==n.responseJSON)throw new Error(r+" error requesting data.")
if(0===n.responseJSON.results.length)throw new Error(r+" no set found.")
return null==s.plotter.plots[t].__data__&&(s.plotter.plots[t].__data__=[]),s.requests[e].ready=!0,s.plotter.plots[t].__data__[i]=n.responseJSON.results,s.plotter.plots[t].proto.skipBandDomainSet=!0,s.plotter.plots[t].proto.setData(s.plotter.plots[t].__data__[i]),s.plotter.plots[t].proto.setBandDomain(s.plotter.bandDomain),s.plotter.plots[t].proto.append(),null==s.plotter.legends[t]&&(s.plotter.legends[t]=new window.Plotter.Legend(s.plotter,t),s.plotter.legends[t].draw()),s.plotter.i.controls.removeSpinner(t)},this.api.get(a,n,o),!0},t.prototype.isReady=function(){var t,i,e,n
t=0,e=this.requests
...
e={dateFormat:"%Y-%m-%dT%H:%M:%SZ"},i=this.mergeDefaults(e,t),this.parseDate=d3.timeParse(i.dateFormat),this.format=d3.utcFormat(i.dateFormat),this.getNow=function(){return this.format(new Date)}}return t.prototype.mergeDefaults=function(t,i){var e,n,s,o,r
s={}
...
for(s in r)l=r[s],this.bands[s]=this.lineWrapper.append("g").attr("clip-path","url(#"+this.options.target+"_clip)").append("path").datum(l).attr("d",this.definition.area).attr("class","line-plot-area-"+s).style("fill",this.options.y[s].color).style("opacity",.15).style("stroke",function(){return d3.color(t.options.y[s].color).darker(1)}),this.lines[s]=this.lineWrapper.append("g").attr("clip-path","url(#"+this.options.target+"_clip)").append("path").datum(l).attr("d",this.definition.line).attr("class","line-plot-path-"+s).style("stroke",this.options.y[s].color).style("stroke-width",Math.round(Math.pow(this.definition.dimensions.width,.1))).style("fill","none")
...
case!(t>=326.25&&t<348.75):return"NNW"
case!(t>=348.75&&t<=360):return"N"}},f=this.data
for(u in f)m=f[u],o=this.overlay.node(),n=m,i=i?i:d3.mouse(o),g=this.definition.x.invert(i[0]+s.leftPadding),t&&(g=this.definition.x.invert(t.invertX(i[0]+s.leftPadding))),h=e.bisectDate(n,g,1),g.getTime()<this.state.range.data[u].min.getTime()&&h--,g.getTime()>this.state.range.data[u].max.getTime()&&h--,h=g.getMinutes()>=30?h:h-1,null!=n[h]&&(p=t?t.applyX(this.definition.x(n[h].x)):this.definition.x(n[h].x),d=[],r=[],null!==this.options.y[u].variable&&(r[u]=n[h],null!=r[u]&&(d[u]=this.definition.y(r[u].y),isNaN(d[u])||null==r[u].y||this.focusCircle[u].attr("transform","translate(0, 0)"))),a=p-s.leftPadding,a>=0&&(this.crosshairs.select(".crosshair-x").attr("x1",a).attr("y1",s.topPadding).attr("x2",a).attr("y2",s.innerHeight+s.topPadding).attr("transform","translate("+s.leftPadding+", 0)"),this.crosshairs.select(".crosshair-x-under").attr("x",a).attr("y",s.topPadding).attr("width",s.innerWidth-a).attr("height",s.innerHeight).attr("transform","translate("+s.leftPadding+", 0)")),null===this.options.y[u].variable||isNaN(d[u])||null==r[u].y||(this.focusCircle[u].attr("cx",p).attr("cy",d[u]),this.focusText[u].attr("x",p+s.leftPadding/10).attr("y",d[u]-s.topPadding/10).text(r[u].y?"wind_direction"===e.options.y[0].variable?l(r[u].y):r[u].y.toFixed(1)+" "+this.options.y[u].units:void 0)))
return i}},t.prototype.showCrosshair=function(){var t,i,e,n
if(this.initialized){this.crosshairs.select(".crosshair-x").style("display",null),this.crosshairs.select(".crosshair-x-under").style("display",null),t=this.options.y,i=[]
...
window.Plotter||(window.Plotter={}),window.Plotter.LiveSync=t=function(){function t(t){this.preError="Plotter.LiveSync",this.plotter=t,this.api=this.plotter.i.api,this.requests={},this.endpoint=function(){return this.plotter.options.href+"/api/v5/measurement"}}return t.prototype.append=function(t,i,e){var n,s,o,r,a,l,p
return n=new Date,e.range.data[i].max>=n||(r=this.plotter.options.updateLength,o=e.range.data[i].max.getTime(),l=new Date(o+36e5*this.plotter.options.updateLength),l>n&&(r=Math.round((l.getTime()-n.getTime())/36e5)),a=this.plotter.lib.format(new Date(l)),r<this.plotter.options.minUpdateLength||(s=this.plotter.i.template.forSync(t,i,a,r),p=this.plotter.lib.uuid(),this.requests[p]={ready:!1,requested:!1},this.requests[p].requested=this.get(t,i,p,s,"max")))},t.prototype.prepend=function(t,i,e){var n,s,o,r
...
return n},t.prototype.listen=function(t){var i,e,n,s,o,r,a
s=this.plots
for(n in s)if(e=s[n],null!=e&&e.proto.initialized){a=e.proto.getState(),o=a.request.data
for(i in o)r=o[i],r.min===!0&&this.isReady()&&e.proto.state.requested.data[i].min===!1&&(this.updates++,e.proto.state.requested.data[i].min=!0,this.i.livesync.prepend(n,i,a)),r.max===!0&&this.isReady()&&e.proto.state.requested.data[i].max===!1&&(this.updates++,e.proto.state.requested.data[i].max=!0,this.i.livesync.append(n,i,a))}if(!t)return setTimeout(Plotter.Handler.prototype.listen.bind(this),this.options.refresh)},t.prototype.append=function(){var t,i,e,n,s
t=this,n=this.plots
for(e in n)s=n[e],s.uuid=this.lib.uuid(),$(this.options.target).append('<div id="outer-'+s.uuid+'"></div>'),i=this.i.template.forPlots(e),i=this.i.colors.getInitial(i),i.target="#outer-"+s.uuid,i.uuid=s.uuid,null!=this.options.width&&(i.width=this.options.width),"bar"===i.plotType?s.proto=new window.Plotter.BarPlot(this,s.__data__,i):s.proto=new window.Plotter.LinePlot(this,s.__data__,i),s.proto.preAppend(),s.proto.append(),this.i.controls.append(e),this.legends[e]=new window.Plotter.Legend(this,e)
...
if(o=this.i.template.template,n=o[t],e=n.pageOrder,a=$(this.plots[t].proto.options.target),"up"===i){if(e>1)return r=this.lib.indexOfValue(o,"pageOrder",e-1),s=o[r],n.pageOrder--,s.pageOrder++,a.prev().insertAfter(a)}else if("down"===i&&e<o.length)return r=this.lib.indexOfValue(o,"pageOrder",e+1),s=o[r],n.pageOrder++,s.pageOrder--,a.next().insertBefore(a)},t.prototype.add=function(t,i){var e,n,s,o,r,a,l,p
return p=this.lib.uuid(),o="outer-"+p,l={pageOrder:this.i.template.plotCount()+1,type:t,target:"#"+o,y:[]},a='<div id="'+o+'"></div>',$(this.options.target).append(a),e=this.i.template.add(l),this.plots[e]={},n=this.i.specs.getPlotType(i),"precipitation_line"===i&&(i="precipitation",n="line"),"bar"===n?this.plots[e].proto=new window.Plotter.BarPlot(this,[[]],l):this.plots[e].proto=new window.Plotter.LinePlot(this,[[]],l),this.plots[e].proto.preAppend(),this.plots[e].proto.options.plotType=n,this.plots[e].proto.options.plotId=e,this.plots[e].proto.options.uuid=p,this.appendSave(),r=this.i.specs.getOptions(i,null),this.i.template.template[e].x=$.extend(!0,{},this.i.template.template[0].x),this.i.template.template[e].y=[r],s=this.i.template.forPlots(e),this.plots[e].proto.options.x=s.x,this.plots[e].proto.options.y=s.y,this.i.controls.append(e)},t.prototype.addStation=function(t,i){var e,n,s,o
return this.plots[t].proto.initialized?"bar"===this.plots[t].proto.options.plotType?(this.i.controls.removeSpinner(t),alert("The bar plot only supports one station. Please add precipitation as a line plot to use multiple."),!1):(e=this.plots[t].proto.getState(),o=e.range.data[0].max.getTime(),n=$.extend(!0,{},this.plots[t].proto.options.y[0]),n.dataLoggerId=i,n.color=this.i.colors.get(i),s=this.plots[t].proto.options.y.push(n)-1,this.i.controls.updateStationDropdown(t),this.i.controls.updateStationMap(t),this.i.livesync.add(t,s,e),!0):(this.plots[t].proto.options.y[0].dataLoggerId=i,this.plots[t].proto.options.y[0].color=this.i.colors.get(i),this.i.initialsync.add(t),this.i.controls.updateStationDropdown(t),this.i.controls.updateStationMap(t),!0)},t.prototype.removeStation=function(t,i){var e
return e=this.lib.indexOfValue(this.plots[t].proto.options.y,"dataLoggerId",i),e>0&&(delete this.i.template.template[t].y[e],this.plots[t].proto.removeData(e),this.plots[t].proto.getDefinition(),this.plots[t].proto.update(),this.i.controls.updateStationDropdown(t),this.i.controls.updateStationMap(t)),this.i.controls.removeSpinner(t)},t.prototype.appendSave=function(){if($("#save-"+this.options.uuid).parent().remove(),this.isAdmin()||null!=this.options.uuid)return $(this.options.target).append('<small><a style="cusor:pointer" id="save-'+this.options.uuid+'">Save Template</a></small>'),$("#save-"+this.options.uuid).on("click",function(t){return _.i.template.put()})},t}()}.call(this),function(){var t
...
return i={battery_voltage:{min:8,max:16,maxBar:null},temperature:{min:null,max:null,maxBar:32},equip_temperature:{min:null,max:null,maxBar:null},relative_humidity:{min:0,max:102,maxBar:null},net_solar:{min:0,max:800,maxBar:null},snow_depth:{min:null,max:null,maxBar:null},snowfall_24_hour:{min:0,max:40,maxBar:null},intermittent_snow:{min:null,max:null,maxBar:null},wind_direction:{min:0,max:360,maxBar:null},precipitation:{min:0,max:.7,maxBar:null},wind_speed_average:{min:0,max:null,maxBar:null},solar_pyranometer:{min:0,max:null,maxBar:null},barometric_pressure:{min:null,max:null,maxBar:null}},i[t]},t.prototype.getVariableInfo=function(t){var i
...
return!0},this.parse=function(t){var i,e,n,s
if(i=JSON.parse(t).templateData,this.isValid(i)){for(e=0,n=i.length;e<n;e++)s=i[e],s.x.min=new window.Plotter.Now(this.plotter.lib.format,s.x.min).get(),s.x.max=new window.Plotter.Now(this.plotter.lib.format,s.x.max).get()
...
return t.template=t.parse(i.responseJSON.template_data)},this.sapi.get(s,i,e)},t.prototype.put=function(){var t,i,e,n,s
...
return s=this.endpoint(),i={id:this.plotter.options.templateId,template_data:this.stringify(this.template)},t=this,e=function(t){return console.log("Template PUT completed (data)",t)},this.api.put(s,i,e)},t.prototype.add=function(t){var i,e,n
...
return e="line",void 0!==this.template[t].plotType&&(e=this.template[t].plotType),a={plotId:t,plotType:e,x:n,y:s}},t.prototype.removePlot=function(t){return delete this.template[t]},t}()}.call(this),function(){var t
