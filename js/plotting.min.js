...
for(s in r)l=r[s],this.bars[s]=this.barWrapper.append("g").attr("clip-path","url(#line-plot-clip-path)").selectAll(".bar-"+s).data(l).enter().append("rect").attr("class","bar-"+s).attr("x",function(i){return t.definition.x(i.x)}).attr("width",d3.max([1,this.definition.x1.bandwidth()])).attr("y",function(i){return t.definition.y(i.y)}).attr("height",function(i){return t.definition.dimensions.innerHeight+t.definition.dimensions.margin.top-t.definition.y(i.y)}).style("fill",this.options.y[s].color)
...
for(s in r)l=r[s],this.bands[s]=this.lineWrapper.append("g").attr("clip-path","url(#line-plot-clip-path)").append("path").datum(l).attr("d",this.definition.area).attr("class","line-plot-area-"+s).style("fill",this.options.y[s].color).style("opacity",.15).style("stroke",function(){return d3.color(t.options.y[s].color).darker(1)}),this.lines[s]=this.lineWrapper.append("g").attr("clip-path","url(#line-plot-clip-path)").append("path").datum(l).attr("d",this.definition.line).attr("class","line-plot-path-"+s).style("stroke",this.options.y[s].color).style("stroke-width",Math.round(Math.pow(this.definition.dimensions.width,.1))).style("fill","none")
