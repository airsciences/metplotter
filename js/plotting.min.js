...
return this.appendCrosshairTarget(n),n}},t.prototype.setCrosshair=function(t,i){var e,n,s,o,r,a,l,p,h,d,u,c,f,m,g,y,x,v,w
...
for(m in y)x=y[m],r=this.overlay.node(),s=x,i=i?i:d3.mouse(r),w=this.definition.x.invert(i[0]+o.leftPadding),t&&(w=this.definition.x.invert(t.invertX(i[0]+o.leftPadding))),f=e.bisectDate(s,w,1),c=w.getMinutes()>=30?f:f-1,w.getTime()<=this.state.range.data[m].min.getTime()&&(c=f),w.getTime()>=this.state.range.data[m].max.getTime()&&(c=f-1),null!=s[c]&&(h=t?t.applyX(this.definition.x(s[c].x)):this.definition.x(s[c].x),d=[],a=[],null!==this.options.y[m].variable&&(a[m]=s[c],null!=a[m]&&(d[m]=this.definition.y(a[m].y),n=this.displayDate(a[m].x),isNaN(d[m])||null==a[m].y||this.focusRect[m].attr("transform","translate(0, 0)"))),l=h-o.leftPadding,l>=0&&(this.crosshairs.select(".crosshair-x").attr("x1",l).attr("y1",o.topPadding).attr("x2",l).attr("y2",o.innerHeight+o.topPadding).attr("transform","translate("+o.leftPadding+", 0)"),this.crosshairs.select(".crosshair-x-under").attr("x",l).attr("y",o.topPadding).attr("width",o.innerWidth-l).attr("height",o.innerHeight).attr("transform","translate("+o.leftPadding+", 0)"),u=l-120,this.focusDateText.attr("x",u).attr("y",o.topPadding+o.innerHeight-3).attr("transform","translate("+o.leftPadding+", 0)").text(n),null===this.options.y[m].variable||isNaN(d[m])||null==a[m].y||(this.focusRect[m].attr("width",t.k*this.definition.x1.bandwidth()).attr("x",h).attr("y",d[m]),v="start",this.focusText[m].attr("x",h+o.leftPadding/10+t.k*this.definition.x1.bandwidth()+2).attr("y",d[m]-o.topPadding/10).attr("text-anchor",v).text(null!=a[m].y?"wind_direction"===e.options.y[0].variable?p(a[m].y):a[m].y.toFixed(this.options.decimals)+" "+this.options.y[m].units:void 0))))
...
return this.appendCrosshairTarget(n),n}},t.prototype.setCrosshair=function(t,i){var e,n,s,o,r,a,l,p,h,d,u,c,f,m,g,y,x,v,w,b
if(this.initialized){g=this.preError+".setCrosshair(mouse)",e=this,o=this.definition.dimensions,p=function(t){switch(!1){case!(t>360||t<0):return"INV"
case!(t>=0&&t<11.25):return"N"
case!(t>=11.25&&t<33.75):return"NNE"
case!(t>=33.75&&t<56.25):return"NE"
...
case!(t>=326.25&&t<348.75):return"NNW"
case!(t>=348.75&&t<=360):return"N"}},y=this.data
for(m in y)x=y[m],r=this.overlay.node(),s=x,i=i?i:d3.mouse(r),w=this.definition.x.invert(i[0]+o.leftPadding),t&&(w=this.definition.x.invert(t.invertX(i[0]+o.leftPadding))),f=e.bisectDate(s,w,1),c=w.getMinutes()>=30?f:f-1,w.getTime()<=this.state.range.data[m].min.getTime()&&(c=f),w.getTime()>=this.state.range.data[m].max.getTime()&&(c=f-1),null!=s[c]&&(h=t?t.applyX(this.definition.x(s[c].x)):this.definition.x(s[c].x),d=[],a=[],null!==this.options.y[m].variable&&(a[m]=s[c],null!=a[m]&&(d[m]=this.definition.y(a[m].y),n=this.displayDate(a[m].x),isNaN(d[m])||null==a[m].y||this.focusCircle[m].attr("transform","translate(0, 0)"))),l=h-o.leftPadding,l>=0&&(this.crosshairs.select(".crosshair-x").attr("x1",l).attr("y1",o.topPadding).attr("x2",l).attr("y2",o.innerHeight+o.topPadding).attr("transform","translate("+o.leftPadding+", 0)"),this.crosshairs.select(".crosshair-x-under").attr("x",l).attr("y",o.topPadding).attr("width",o.innerWidth-l).attr("height",o.innerHeight).attr("transform","translate("+o.leftPadding+", 0)"),u=l-120,this.focusDateText.attr("x",u).attr("y",o.topPadding+o.innerHeight-3).attr("transform","translate("+o.leftPadding+", 0)").text(n),null===this.options.y[m].variable||isNaN(d[m])||null==a[m].y||(this.focusCircle[m].attr("cx",h).attr("cy",d[m]),v="start",this.focusText[m].attr("x",h+o.leftPadding/10).attr("y",d[m]-o.topPadding/10).attr("text-anchor",v).text(a[m].y?"wind_direction"===e.options.y[0].variable?p(a[m].y):a[m].y.toFixed(this.options.decimals)+" "+this.options.y[m].units:void 0))))
...
return n=new Date,o=n.getTimezoneOffset()/60,s=new Date(n.getTime()-36e5*o),e.range.data[i].max>=s||(l=this.plotter.options.updateLength,a=e.range.data[i].max.getTime(),h=new Date(a+36e5*this.plotter.options.updateLength),h>s&&(l=Math.round((h.getTime()-s.getTime())/36e5)),p=this.plotter.lib.format(new Date(h)),l<this.plotter.options.minUpdateLength||(r=this.plotter.i.template.forSync(t,i,p,l),d=this.plotter.lib.uuid(),this.requests[d]={ready:!1,requested:!1},this.requests[d].requested=this.get(t,i,d,r,"max")))},t.prototype.prepend=function(t,i,e){var n,s,o,r
...
if(o=this.i.template.template,n=o[t],e=n.pageOrder,a=$(this.plots[t].proto.options.target),"up"===i){if(e>1)return r=this.lib.indexOfValue(o,"pageOrder",e-1),s=o[r],n.pageOrder--,s.pageOrder++,a.prev().insertAfter(a)}else if("down"===i&&e<o.length)return r=this.lib.indexOfValue(o,"pageOrder",e+1),s=o[r],n.pageOrder++,s.pageOrder--,a.next().insertBefore(a)},t.prototype.add=function(t,i){var e,n,s,o,r,a,l,p,h,d,u,c,f
for(f=this.lib.uuid(),r="outer-"+f,d={width:this.options.width,pageOrder:this.i.template.plotCount()+1,type:t,target:"#"+r,requestInterval:this.options.requestPaddingInterval,y:[]},l='<div id="'+r+'"></div>',$(this.options.target).append(l),n=this.i.template.add(d),this.plots[n]={},s=this.i.specs.getPlotType(i),"precipitation_line"===i&&(i="precipitation",s="line"),e=this.i.specs.getPlotDecimals(i),console.log("New Plot (variable, decimals): ",i,e),"bar"===s?this.plots[n].proto=new window.Plotter.BarPlot(this,[[]],d):this.plots[n].proto=new window.Plotter.LinePlot(this,[[]],d),this.plots[n].proto.preAppend(),this.plots[n].proto.options.plotType=s,this.plots[n].proto.options.plotId=n,this.plots[n].proto.options.uuid=f,this.plots[n].proto.options.decimals=e,this.appendSave(),a=this.i.specs.getOptions(i,null),u=this.i.template.template,p=0,h=u.length;p<h;p++)if(c=u[p],void 0!==c){this.i.template.template[n].x=$.extend(!0,{},c.x)
...
return e="line",void 0!==this.template[t].plotType&&(e=this.template[t].plotType),a=$.extend(!0,{},this.template[t]),a.plotId=t,a.plotType=e,a.x=n,a.y=s,a},t.prototype.removePlot=function(t){return delete this.template[t]},t.prototype.resetPageOrder=function(t){var i,e
